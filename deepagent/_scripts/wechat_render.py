...
品牌主题从外部 brand.md 读取（深空 + 青/品红折射为默认），见 brand.example.md。
"""
import argparse
import glob
import html
...
# ── brand-driven: read external brand.md via deepagent.brand; deep-space defaults ──
try:
    from deepagent.brand import load_brand as _load_brand
    _B = _load_brand()
except Exception:
    _B = None
if _B and _B.present:
    CY = _B.palette.get("cyan", CY)
    MG = _B.palette.get("magenta", MG)
    VI = _B.palette.get("purple", VI)
    INK_DEEP = _B.palette.get("bg_start", INK_DEEP)
_NAME = (_B.name if _B and _B.present else "") or "Brand"
_SLOGAN_FULL = (_B.slogan if _B and _B.slogan else "")
_SLOGAN_KEY = (_SLOGAN_FULL.split("，")[0] if _SLOGAN_FULL else "") or _NAME
_SITE = (_B.site if _B and _B.present else "")
...
CAPTIONS = {}  # native ![](file.png) images carry their own caption via alt text

# 末尾品牌收尾图：一次生成、所有文章固定复用（不再每篇重生成）。
# 见 platform-writing/references/wechat.md 的配图规范。
try:
    from deepagent import config as _cfg
    WORKSPACE = str(_cfg.WORKSPACE_ROOT)
except Exception:
    WORKSPACE = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
BRAND_SIGNOFF = os.path.join(WORKSPACE, "brand", "assets", "wechat-signoff.png")
...
def signoff_card():
    return (f'<div style="margin:40px 0 26px;padding:26px 20px;text-align:center;'
            ...
            f'<div style="font-size:22px;font-weight:800;color:{CY};letter-spacing:2px;">{_NAME}</div>'
            ...
            f'letter-spacing:1px;">{_SITE}</div></div>')
    ...
    for b in blocks:
        ...
        if kind == "h":
            ...
            if level == 1:
                ...
                out.append(f'<div style="{S_KICKER}">{_NAME} · 深度解读</div>')
            else:
        ...
        if kind == "quote":
            ...
            blob = " ".join(lines)
            # 品牌收尾（slogan 关键词 + 品牌名触发）
            if _SLOGAN_KEY and _SLOGAN_KEY in blob and _NAME in blob:
                section_image("ending", "")
                out.append(signoff_card()); continue
            if any(k in blob for k in ("关注", "在看", "转发")) and \
                    (_NAME in blob or _SITE in blob):
        ...
        if kind == "p":
            ...
            if _SLOGAN_KEY and _SLOGAN_KEY in stripped and _NAME in stripped:
                section_image("ending", "")
                out.append(signoff_card()); continue
